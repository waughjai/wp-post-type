...
namespace WaughJ\WPPostType
{
	class WPPostType
	{
		public function __construct( string $slug, string $name, array $args = [] )
		{
			$this->slug = $slug;
			$this->name = $name;
			$this->other_arguments = new VerifiedArgumentsSameType( $args, $this->argumentDefaults() );
		...
		public function getMessages( array $messages ) : array
		{
			...
			[
				...
				8 => sprintf( __($this->other_arguments->Get( 'singular_name' ) . ' submitted. <a target="_blank" href="%s">Preview book</a>'), esc_url( add_query_arg( 'preview', 'true', get_permalink($post_ID) ) ) ),
		...
		private function registerMetaBoxes() : void
		{
			...
			foreach ( $meta_box_data as $meta_box_item )
			{
				...
				if ( $name && $slug )
				{
					...
					array_push( $this->meta_boxes, new MetaBox( $full_slug, $name, $type, $input_type, $meta_box_item ) );
				}
			}
		}
